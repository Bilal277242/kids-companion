...
describe('the parent dashboard', () => {
  ...
  describe('GET /api/parent/subscription', () => {
    it('reports the free plan when there is no subscription', async () => {
      const response = await harness.app.inject({
        method: 'GET',
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json<{
        plan: { code: string; tier: string; status: string; priceMinor: number };
        limits: { childProfileLimit: number; dailyTurnLimit: number };
        childProfilesUsed: number;
        availablePlans: { code: string }[];
      }>();

      expect(body.plan.tier).toBe('free');
      expect(body.plan.status).toBe('free');
      // A free plan is free — the constraint says so in the database, and a
      // dashboard showing a price against it would be a pricing bug in public.
      expect(body.plan.priceMinor).toBe(0);
      expect(body.limits.childProfileLimit).toBeGreaterThan(0);
      expect(body.childProfilesUsed).toBeGreaterThan(0);
      expect(body.availablePlans.length).toBeGreaterThan(0);
    });

    it('counts only this parent’s own children against the plan limit', async () => {
      // Bob has a child too. Alice's count must not include him, and the query
      // that produces it has no parent filter of its own — RLS is what makes
      // this true, so it is worth asserting rather than assuming.
      const response = await harness.app.inject({
        method: 'GET',
        url: '/api/parent/subscription',
        headers: authHeader(alice.accessToken),
      ...
      const { rows } = await harness.db.query<{ n: number }>(
        'select count(*)::int as n from children where parent_id = $1 and deleted_at is null',
        [alice.parentId],
      );

      expect(response.json<{ childProfilesUsed: number }>().childProfilesUsed).toBe(rows[0]?.n);
    });

    it('never returns a card number, a vendor token, or an internal identifier', async () => {
      const response = await harness.app.inject({
        method: 'GET',
        url: '/api/parent/subscription',
        headers: authHeader(alice.accessToken),
      ...
      // The response schema decides what leaves this API. A brand and four
      // digits are all the shape allows, and there is no column for more.
      const body = response.json<{ paymentMethod: Record<string, unknown> }>();
      expect(Object.keys(body.paymentMethod).sort()).toEqual(['brand', 'last4']);
    });
